...
export function reducerWithLocalStorage(domainModel, command) {
    ...
    try {
        ...
        return new LocationTracker(dm.TrackingService, dm.Visualization, dm.Tracks, _Error);
    }
}
export function start() {
    var initialStoreState = new LocationTracker(new TrackingService(new LocationService(window.location.origin)), new TrackVisualization("", TrackVisualization.calculate(ofArray([new TrackingPoint(8.9, 5.9, now(), 34.9, 0, 32300.9, 0, 320.3), new TrackingPoint(9.9, 5.9, now(), 34.9, 0, 32300.9, 1, 32.3)]))), ofArray([new Track(now(), function () {
